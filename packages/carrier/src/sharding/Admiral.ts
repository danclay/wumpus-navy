...
export interface ClusterStats<LibLatencyRef> {
	id: number;
	guilds: number;
	/** Cached users */
	users: number;
	...
	requestHandlerLatencyRef?: LibLatencyRef;
}
...
export interface Stats<LibLatencyRef> {
	guilds: number;
	/** Total cached users */
	users: number;
...
*/
export class Admiral<LibClient, LibClientType, LibClientOptions, LibLatencyRef, LibClientStatus> extends EventEmitter {
	...
	private prelimStats?: Stats<LibLatencyRef>;
	...
	private startingStatus?: LibClientStatus;
	...
	/** Adapted cluster for lib @internal */
	public AdaptedCluster?: typeof Cluster<LibClient, LibClientType, LibClientOptions, LibLatencyRef, LibClientStatus>;
	/** Gateway recommended shards @internal */
	public getBotGateway?(): Promise<GetBotGateway>;
	/** @internal */
				...
				if (message.op) {
					...
					case "collectStats": {
						...
						if (this.statsWorkersCounted === this.clusters.size + this.services.size) {
								...
								const compare = (a: ClusterStats<LibLatencyRef>, b: ClusterStats<LibLatencyRef>) => {
						...
						const concurrency = () => {
							if (item.type === "service" && this.startServicesTogether && this.queue.queue[1]) {
								...
								if (this.queue.queue[1].type === "cluster") {
									const currentClusterID = (item.message as ClusterConnectMessage<unknown>).clusterID;
									const nextClusterID = (this.queue.queue[1].message as ClusterConnectMessage<unknown>).clusterID;
									const clusterToGroupMap = this.chunkConcurrencyGroups();
	...
	public launch() {
		...
		if (nodeCluster.isMaster) {
			...
			if (process.env.type === "cluster") {
				new this.AdaptedCluster!({
					LibClient: this.LibClient!,
					fetchTimeout: this.fetchTimeout,
	...
	*/
	 ...
	 */
	public collectStats(): Promise<Stats<LibLatencyRef>> {
		this.executeStats();
		return new Promise((res) => {
			this.once("stats", (stats: Stats<LibLatencyRef>) => {
				res(stats);
			});
		});
	}
	...
	private executeStats() {
		...
		this.prelimStats = {
			...
			services: [],
			timestamp: 0,
			centralRequestHandlerLatencyRef: this.useCentralRequestHandler ? this.getCentralRequestHandlerLatencyRef!() : undefined
		};
